...
public class Vaccines extends Resource implements Production {
    ...
    public void addQuantity(Planet planet, Integer quantity, Boolean newlyCreated) {
        if (quantity > 0) {
            ...
            planet.changeQuantity_Vaccines(quantity);
    ...
    public void setDemanded(Planet planet) {
        if(planet.isDemanded_Vaccines().equals(false)) {
            planet.setDemanded_Vaccines(true);
    ...
    public void setNotDemanded(Planet planet) {
        if(planet.isDemanded_Vaccines().equals(true)) {
            planet.setDemanded_Vaccines(false);
