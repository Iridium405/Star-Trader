...
public class Dioptase extends Resource implements Extraction {
    ...
    public void addQuantity(Planet planet, Integer quantity, Boolean newlyCreated) {
        if (quantity > 0) {
            ...
            planet.changeQuantity_Dioptase(quantity);
    ...
    public void setDemanded(Planet planet) {
        if(planet.isDemanded_Dioptase().equals(false)) {
            planet.setDemanded_Dioptase(true);
    ...
    public void setNotDemanded(Planet planet) {
        if(planet.isDemanded_Dioptase().equals(true)) {
            planet.setDemanded_Dioptase(false);
