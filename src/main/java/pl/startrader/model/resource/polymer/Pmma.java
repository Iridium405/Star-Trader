...
public class Pmma extends Resource implements Production {
    ...
    public void addQuantity(Planet planet, Integer quantity, Boolean newlyCreated) {
        if (quantity > 0) {
            ...
            planet.changeQuantity_PMMA(quantity);
    ...
    public void setDemanded(Planet planet) {
        if(planet.isDemanded_PMMA().equals(false)) {
            planet.setDemanded_PMMA(true);
    ...
    public void setNotDemanded(Planet planet) {
        if(planet.isDemanded_PMMA().equals(true)) {
            planet.setDemanded_PMMA(false);
