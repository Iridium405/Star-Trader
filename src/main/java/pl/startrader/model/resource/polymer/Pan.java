...
public class Pan extends Resource implements Production {
    ...
    public void addQuantity(Planet planet, Integer quantity, Boolean newlyCreated) {
        if (quantity > 0) {
            ...
            planet.changeQuantity_PAN(quantity);
    ...
    public void setDemanded(Planet planet) {
        if(planet.isDemanded_PAN().equals(false)) {
            planet.setDemanded_PAN(true);
    ...
    public void setNotDemanded(Planet planet) {
        if(planet.isDemanded_PAN().equals(true)) {
            planet.setDemanded_PAN(false);
