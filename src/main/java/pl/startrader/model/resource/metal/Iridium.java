...
public class Iridium extends Resource implements Extraction {
    ...
    public void addQuantity(Planet planet, Integer quantity, Boolean newlyCreated) {
        if (quantity > 0) {
            ...
            planet.changeQuantity_Iridium(quantity);
    ...
    public void setDemanded(Planet planet) {
        if(planet.isDemanded_Iridium().equals(false)) {
            planet.setDemanded_Iridium(true);
    ...
    public void setNotDemanded(Planet planet) {
        if(planet.isDemanded_Iridium().equals(true)) {
            planet.setDemanded_Iridium(false);
