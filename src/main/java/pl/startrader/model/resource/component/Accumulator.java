...
public class Accumulator extends Resource implements Production {
    ...
    public void addQuantity(Planet planet, Integer quantity, Boolean newlyCreated) {
        if (quantity > 0) {
            ...
            planet.changeQuantity_Accumulator(quantity);
    ...
    public void setDemanded(Planet planet) {
        if(planet.isDemanded_Accumulator().equals(false)) {
            planet.setDemanded_Accumulator(true);
    ...
    public void setNotDemanded(Planet planet) {
        if(planet.isDemanded_Accumulator().equals(true)) {
            planet.setDemanded_Accumulator(false);
